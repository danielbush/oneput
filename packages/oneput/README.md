...
This is also pre-alpha but potentially usable.
...
Import the defaults stylesheet once in the host app (see `apps/oneput-demo/src/routes/+layout.svelte`):

```ts
import '@oneput/oneput/shared/styles/oneput-defaults.css';
```

That file is the single entry for Oneput chrome. Cascade layers are ordered:
...
- `<OneputController>` wraps `<Oneput>`, feeds props to `<Oneput>` and manages them as state; creates a `Controller` object that encapsulates access to this state and then provides this to the consumer via a callback called `run`. This is the thing most consumers will see and use.
