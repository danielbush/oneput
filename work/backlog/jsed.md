...
- [x] selections
  ...
  - across p-tags
- [ ] split elements
  - Nested INLINE_FLOW chains, e.g. `<p>aaa <em>bbb <strong>ccc</strong> ddd</em> eee</p>`. The fix should split every LINE_SIBLING ancestor from token parent up to the
    LINE, not just one level.
  - Splitting at INLINE_FLOW boundaries, e.g. first token inside `<em>` for splitBefore, last token inside `<em>` for splitAfter. One side may become an empty inline
    element; we need to decide whether it gets removed, anchored, or preserved.
  - Mixed ISLAND siblings inside an INLINE_FLOW, e.g. `<em>aaa <span class="katex">x</span> bbb</em>`. The split should move ISLANDs to the correct side without
    descending into them.
  - Root-level text-only splits for splitBefore as well as splitAfter. You asked for splitAfter; the mirror probably matters too if Enter-before-current is possible
    at root text level.
