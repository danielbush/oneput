---
id: RENAME_CURSOR_CLASSES__WORK
status: done
package: jsed
created: 2026-03-18
outcome: Renamed all four cursor CSS class string values in constants.ts and updated all selectors in jsed-defaults.css. All checks and tests pass.
---
...
- **2026-03-25** — renamed CSS class string values in `constants.ts` and updated all selectors in `jsed-defaults.css`. Also updated the comment block referencing the old names. `task check` and `task test` both pass (155 jsed tests, 0 errors).
